...
import { getEpaileakDB } from "../html/js/controller";
class user {
...
function lehiaketaSortuDB(lehiaketa_){
    const query = 'INSERT INTO `putxerappdb`.`txapelketa` (`izena`, `dataOrdua`, `lekua`) VALUES ('+lehiaketa_.lehiaketaIzena+', '+lehiaketa_.data+', '+lehiaketa_.lekua+')';

}

function findLehiaketa(lehiaketaIzena){
...
function checkCheckbox(epaileak){
...
function lehiaketaBistaratu(){
    lehiaketakTmp();
    //lehiaketak = JSON.parse(localStorage.getItem("lehiaketak"));
    if(lehiaketak == null){
